 ...
 * This one is just biased to a only keep even rank cards
 * Will pass every card that ISN'T the an even rank.
 */
...
public class TyeroneEvenStevenPlayer implements SpoonsStrategy {
  ...
  public TyeroneEvenStevenPlayer(String name) {
    this.name = name;
  }
  ...
  public Card decideCardToPass() {
    ...
    // When there is an overflow_card check through each card in hand to see if it is even rank
    for (int i = 0; i < 4; i++) {
      if (hand[i] == null) continue; // Pass over null cards in hand
      boolean is_even = false;
      
      for (String rank: even_ranks) { // Iterate over each even rank
      ...
      if (!is_even) { // If the card in hand isnt even pass it
         card_to_pass = hand[i];
    ...
    // In the case of a full hand of evens randomly choose a card in hand and discard that    
  ...
  public boolean hasFourOfAKind() {
    int[] rank_counts = new int[even_ranks.length]; // Create a parallel array to even_ranks
    // Checks if the current hand has a four of a kind of the fav_rank
    for (Card card: hand) {
        if (card == null) continue; // Pass over any null hand cards
        for (int i = 0; i< even_ranks.length; i++){
            if (card.getRank().equals(even_ranks[i])) { // Checks if any card in hand is in odd_ranks
